...
dash.register_page(__name__, path="/channels", name="Channels", order=10)
...
def layout():
    ...
    return html.Div(
        children=[
            # Auth state stores
            dcc.Store(id="telegram-auth-state", data="idle"),
                ...
                [
                    ...
                    dbc.Col(
                        [
                            ...
                            # Google Calendar API
                            _channel_card(
                                "bi bi-camera-video", "Google Calendar API",
                                "Connect a Google Service Account to create calendar events with Meet links when meeting drafts are sent.",
                                [
                                    html.Div(
                                        id="gcal-api-connection-status",
                                        style={"marginBottom": "12px"},
                                    ),
                                    html.P(
                                        "Upload a Google Service Account JSON key file with domain-wide delegation enabled. "
                                        "The delegated user will be your IMAP email address.",
                                        style={"color": COLORS["text_muted"], "fontSize": "0.8rem", "marginBottom": "12px",
                                               "background": COLORS["body_bg"], "padding": "10px", "borderRadius": "6px"},
                                    ),
                                    dcc.Upload(
                                        ...
                                        children=html.Div([
                                            html.I(className="bi bi-cloud-arrow-up", style={"fontSize": "1.5rem", "marginRight": "8px"}),
                                            "Drop service account JSON key here",
                                        ]),
                                        style={
                                            "width": "100%",
                                            "borderWidth": "2px",
                                    ...
                                    dcc.Loading(
                                        type="dot", color=COLORS["info"],
                                        children=[html.Div(id="gcal-api-status", style={"marginTop": "8px"})],
                                    ),
                                ],
                                border_color=COLORS["info"],
                            ),
...
@callback(
    Output("gcal-api-connection-status", "children"),
    Input("gcal-api-status", "children"),
    Input("gcal-key-upload", "contents"),
)
def gcal_api_connection_status(*_):
            ...
            return html.Div(
                ...
                children=[
                    html.I(className="bi bi-circle-fill",
                           style={"fontSize": "0.6rem", "color": COLORS["success"]}),
                    html.Span(f"Connected — {email}",
                              style={"color": COLORS["text_secondary"], "fontSize": "0.85rem"}),
                ],
            )
        except Exception:
    ...
    return html.Div(
        ...
        children=[
            html.I(className="bi bi-circle-fill",
                   style={"fontSize": "0.6rem", "color": COLORS["text_muted"]}),
...
# Google Calendar API key upload
@callback(
    Output("gcal-api-status", "children"),
    Input("gcal-key-upload", "contents"),
    State("gcal-key-upload", "filename"),
    prevent_initial_call=True,
)
def handle_gcal_key_upload(contents, filename):
    if not contents:
        return no_update
    ...
    try:
        ...
        return _status_badge("Failed to parse JSON key file.", "danger")

    # Validate it looks like a service account key
    if "client_email" not in key_data or "private_key" not in key_data:
        return _status_badge("Invalid service account key — missing client_email or private_key.", "danger")

    db.save_setting("google_service_account_key", decoded)
    return _status_badge(
        f"Service account key saved — {key_data['client_email']}. Calendar integration is active.",
        "success",
    )

...
dash.register_page(__name__, path="/drafts", name="Drafts", order=13)
    ...
    if read_only:
        children.append(
            html.Div(
                ...
                children=[
                    html.I(className="bi bi-info-circle-fill", style={"color": COLORS["info"], "fontSize": "1.1rem"}),
                    html.Span(
                        "Read-only mode is active. Outbound communication is disabled.",
                        style={"color": COLORS["info"], "fontSize": "0.9rem"},
                    ),
                ],
            )
        )
...
def render_drafts(filter_clicks, _refresh):
    ...
    for draft in drafts:
        ...
        # Action buttons — hidden when read-only or terminal status
        action_buttons = None

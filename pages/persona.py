...
auto-reply configuration, chat export ingestion.
"""
...
dash.register_page(__name__, path="/persona", name="Persona", order=8)
                    ...
                    dbc.Col(
                        [
                            ...
                            html.Div(
                                ...
                                children=[
                                    html.H4("Auto-Reply Settings", style={"color": COLORS["text_primary"], "marginBottom": "16px"}),
                                    html.Div(
                                        style={"display": "flex", "alignItems": "center", "gap": "12px", "marginBottom": "16px"},
                                        children=[
                                            dbc.Switch(
                                                id="persona-auto-reply-toggle",
                                                value=auto_reply,
                                    ...
                                    html.P(
                                        "When enabled, high-confidence drafts will be auto-approved (still requires manual send or SMTP integration).",
                                        style={"color": COLORS["text_muted"], "fontSize": "0.8rem", "marginBottom": "16px"},
                                    ),
                                    html.Label(
                                        "Confidence Threshold",
                                        style={"color": COLORS["text_secondary"], "fontSize": "0.85rem", "marginBottom": "4px", "display": "block"},
                                    ),
                                    dcc.Slider(
                                        ...
                                        step=0.05,
                                        value=threshold,
                                        marks={0.5: "0.50", 0.7: "0.70", 0.85: "0.85", 1.0: "1.00"},
                                        tooltip={"placement": "bottom", "always_visible": True},
                                    ),
                                    html.Div(id="persona-config-status", style={"marginTop": "12px"}),
                                ],
                            ),
                        ],
                        md=5,
...
@callback(
    ...
    Input("persona-kpi-row", "children"),  # trigger on page load
...
@callback(
    Output("persona-config-status", "children"),
    Input("persona-auto-reply-toggle", "value"),
    Input("persona-confidence-threshold", "value"),
)
def save_auto_reply_config(enabled, threshold):
    db.save_setting("persona_auto_reply_enabled", "true" if enabled else "false")
    db.save_setting("persona_confidence_threshold", str(threshold))
    status = "ON" if enabled else "OFF"
    return html.Span(
        f"Auto-reply {status}, threshold {threshold:.2f}",
        style={"color": COLORS["text_muted"], "fontSize": "0.8rem"},
    )
